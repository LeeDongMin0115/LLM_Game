...
def progress_node(state: GameState):
    ...
    # 1-5턴: 1차 조사, 6-30턴: 2차 조사
    # 30턴 이전에 범인을 확정할 경우 아래 streamlit UI에서 바로 엔딩으로 넘어가도록 진행
...
with right:
    ...
    st.write(f"현재 턴: {min(state['turn'], 30)} / 30")
...
"""
        ...
        with st.form(key=f"question_form_{state['turn']}_{state['stage']}_{character_key}"):
        ...
        if state["final_choice"] == "alice":
            ...
            for line in ending_text["alice"]:
                st.write(line)
            st.success("빌이 끝내 털어놓은 진실, 앨리스의 지나치게 완벽한 태도, 그리고 체셔 고양이가 흘리던 기묘한 힌트까지... 모든 조각이 마침내 하나의 진실을 가리키고 있었다.")
        else:
            st.subheader("오답 엔딩")
